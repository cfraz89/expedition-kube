...
A [timoni.sh](http://timoni.sh) module for deploying server to Kubernetes clusters.
...
```shell
timoni -n default apply server oci://<container-registry-url>
```
...
```shell
timoni -n default delete server
```
